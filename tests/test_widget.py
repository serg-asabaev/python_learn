...
@pytest.mark.parametrize('date, formated_date', [('2024-03-11T02:26:18.671407', '11.03.2024'),
                                                 ('2025-02-12T02:26:18.671407', '12.02.2025'),
                                                 ('2026-01-11T02:26:18.671407', '11.01.2026')])
def test_get_date(date, formated_date):
    assert widget.get_date(date) == formated_date

def test_get_date_empty():
    assert widget.get_date('') == ''

@pytest.mark.parametrize('card_num, card_mask', [
...
def test_mask_account_card_card(card_num, card_mask):
    assert widget.mask_account_card(card_num) == card_mask


def test_mask_account_card_account_empty():
    assert widget.mask_account_card('') == ''

@pytest.mark.parametrize('acc_num, acc_mask', [
